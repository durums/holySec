...
export const FINDINGS = [
  { id: 'f001', clientId: 'c001', title: 'Log4Shell RCE via Logging-Endpoint', cve: 'CVE-2021-44228', cvss: 10.0, severity: 'CRITICAL', category: 'RCE', status: 'Closed', date: '2025-02-10', description: 'Apache Log4j2 JNDI-Lookup im internen Audit-Service erlaubt Remote Code Execution.', remediation: 'Update Log4j2 auf ≥ 2.17.1, JNDI-Lookup deaktivieren.' },
  { id: 'f002', clientId: 'c001', title: 'HTTP/2 Rapid Reset DDoS Amplification', cve: 'CVE-2023-44487', cvss: 7.5, severity: 'HIGH', category: 'DoS', status: 'In Remediation', date: '2025-03-18', description: 'Nginx-Version anfällig für HTTP/2 Rapid Reset Attack — potenzieller Dienst-Ausfall.', remediation: 'Nginx ≥ 1.25.3 oder Patch einspielen, Rate-Limiting aktivieren.' },
  { id: 'f003', clientId: 'c001', title: 'Stored XSS im Kundenportal', cve: null, cvss: 6.1, severity: 'MEDIUM', category: 'XSS', status: 'Open', date: '2025-04-02', description: 'Feld "Verwendungszweck" im Online-Banking wird unsanitisiert gerendert — gespeichertes XSS möglich.', remediation: 'Content-Security-Policy verschärfen, Eingabe escapen.' },
  { id: 'f004', clientId: 'c001', title: 'Veraltetes TLS 1.0 auf Legacy-API', cve: null, cvss: 5.3, severity: 'MEDIUM', category: 'Crypto', status: 'Open', date: '2025-04-15', description: 'TLS 1.0 und 1.1 noch aktiv auf api-legacy.sparkasse-rl.de — BEAST/POODLE-Angriffe möglich.', remediation: 'TLS 1.0/1.1 deaktivieren, nur TLS 1.2+ mit starken Cipher-Suites.' },
  { id: 'f005', clientId: 'c001', title: 'Exposed Jenkins Build-Server', cve: 'CVE-2024-23897', cvss: 9.8, severity: 'CRITICAL', category: 'Exposure', status: 'Closed', date: '2025-05-01', description: 'Jenkins 2.441 ohne Auth öffentlich erreichbar — File-Read-Vulnerabilität ausnutzbar.', remediation: 'Jenkins hinter VPN, Auth erzwingen, Update auf ≥ 2.443.' },
  { id: 'f006', clientId: 'c002', title: 'SMBGhost auf Fileserver', cve: 'CVE-2020-0796', cvss: 10.0, severity: 'CRITICAL', category: 'RCE', status: 'Closed', date: '2025-04-20', description: 'SMBv3-Komprimierungsfehler auf zentralem Fileserver 172.16.20.5 — ungepatchtes Windows Server 2019.', remediation: 'KB4551762 einspielen, SMBv3-Komprimierung deaktivieren.' },
  { id: 'f007', clientId: 'c002', title: 'Default-Credentials auf DICOM-Server', cve: null, cvss: 8.1, severity: 'HIGH', category: 'Auth', status: 'Open', date: '2025-04-21', description: 'DICOM-Server mit admin:admin erreichbar — direkter Zugriff auf Bildmaterial möglich.', remediation: 'Passwort ändern, Netzwerksegmentierung, DICOM-Server nur intern.' },
  { id: 'f008', clientId: 'c002', title: 'Unverschlüsseltes HL7-Interface', cve: null, cvss: 6.5, severity: 'MEDIUM', category: 'Crypto', status: 'In Remediation', date: '2025-05-10', description: 'HL7-Nachrichten zwischen KIS und Labor werden im Klartext übertragen.', remediation: 'TLS-Wrapper für HL7-MLLP implementieren.' },
  { id: 'f009', clientId: 'c003', title: 'SQL-Injection im Produktfilter', cve: null, cvss: 9.1, severity: 'CRITICAL', category: 'SQLi', status: 'Open', date: '2026-05-12', description: 'GET-Parameter ?category= im Produktkatalog direkt in SQL-Query eingebettet — Blind SQLi bestätigt.', remediation: 'Prepared Statements, WAF-Regel, Input-Validierung.' },
  { id: 'f010', clientId: 'c003', title: 'IDOR im Order-Management', cve: null, cvss: 7.3, severity: 'HIGH', category: 'Auth', status: 'Open', date: '2026-05-14', description: 'Bestellungen anderer Nutzer über /api/orders/{id} ohne Autorisierungsprüfung abrufbar.', remediation: 'RBAC auf API-Ebene erzwingen, Order-IDs nicht vorhersehbar machen.' },
  { id: 'f011', clientId: 'c004', title: 'Apache Struts RCE', cve: 'CVE-2023-50164', cvss: 9.8, severity: 'CRITICAL', category: 'RCE', status: 'Closed', date: '2025-10-03', description: 'Struts 2.5.32 auf internem Formular-Backend anfällig für File-Upload-RCE.', remediation: 'Struts auf 2.5.33+ updaten, Upload-Verzeichnisse nicht ausführbar.' },
  { id: 'f012', clientId: 'c004', title: 'Fehlende HSTS-Header', cve: null, cvss: 4.3, severity: 'LOW', category: 'Config', status: 'Closed', date: '2025-10-10', description: 'HSTS nicht gesetzt — Downgrade-Angriffe auf TLS möglich.', remediation: 'Strict-Transport-Security Header mit includeSubDomains setzen.' },
  { id: 'f013', clientId: 'c004', title: 'Veraltete jQuery-Version mit XSS', cve: 'CVE-2019-11358', cvss: 6.1, severity: 'MEDIUM', category: 'XSS', status: 'Open', date: '2025-11-02', description: 'jQuery 1.8.3 im Frontend — prototype pollution und XSS möglich.', remediation: 'jQuery auf 3.7.1+ updaten oder auf natives JS migrieren.' },
  { id: 'f014', clientId: 'c005', title: 'Kubernetes RBAC Privilege Escalation', cve: 'CVE-2022-3294', cvss: 8.8, severity: 'HIGH', category: 'PrivEsc', status: 'In Remediation', date: '2025-07-20', description: 'Falsch konfigurierte ClusterRoleBindings erlauben SA-Eskalation zu cluster-admin.', remediation: 'RBAC-Audit, least-privilege ServiceAccounts, OPA Gatekeeper.' },
  { id: 'f015', clientId: 'c005', title: 'Container Escape via runc', cve: 'CVE-2024-21626', cvss: 8.6, severity: 'HIGH', category: 'Container', status: 'Open', date: '2025-08-05', description: 'runc 1.1.11 auf Worker-Nodes anfällig — Container-Escape in Host-Namespace möglich.', remediation: 'runc ≥ 1.1.12, keine privilegierten Container ohne Notwendigkeit.' },
  { id: 'f016', clientId: 'c005', title: 'S3-Bucket öffentlich lesbar', cve: null, cvss: 7.5, severity: 'HIGH', category: 'Exposure', status: 'Open', date: '2025-09-12', description: 'Bucket cloudstack-static-assets ohne ACL — Log-Dateien und Configs öffentlich.', remediation: 'S3 Block Public Access aktivieren, Bucket-Policy restriktiv setzen.' },
  { id: 'f017', clientId: 'c005', title: 'GraphQL Introspection in Produktion', cve: null, cvss: 5.3, severity: 'MEDIUM', category: 'Config', status: 'Open', date: '2025-10-01', description: 'GraphQL Introspection aktiviert — vollständiges Schema öffentlich abrufbar.', remediation: 'Introspection in Prod deaktivieren, Depth-Limiting aktivieren.' },
  { id: 'f018', clientId: 'c006', title: 'Modbus TCP ohne Authentifizierung', cve: null, cvss: 8.2, severity: 'HIGH', category: 'OT/ICS', status: 'Open', date: '2026-02-28', description: 'Modbus-Geräte im Segment 10.100.0.0/24 ohne Auth erreichbar — direkte Steuerung möglich.', remediation: 'Netzwerksegmentierung, Modbus-Proxy mit Auth vorschalten.' },
  { id: 'f019', clientId: 'c006', title: 'Passwort-Spray auf Active Directory', cve: null, cvss: 7.0, severity: 'HIGH', category: 'Auth', status: 'Open', date: '2026-03-02', description: 'AD-Lockout-Policy zu permissiv — 5 Versuche / 30min. Spray mit 3 Versuchen/Stunde erfolgreich.', remediation: 'Lockout auf 3 Versuche reduzieren, MFA für alle Accounts, LAPS.' },
]
